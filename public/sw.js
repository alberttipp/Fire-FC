...
//
// NOTE (2026-06-23 incident): a network-first navigation `fetch` handler was
// added here to defeat the stale OS app-shell cache, but intercepting
// navigate-mode requests broke loading on installed PWAs — the shell fell
// through to a near-black offline page (full black-screen outage). Removed.
// If we retry app-shell freshness via the SW, fetch by URL string
// (`fetch(req.url, { cache: 'no-store', redirect: 'follow' })`) and TEST on a
// real installed iOS/Android PWA first — never re-fetch the navigate Request.
