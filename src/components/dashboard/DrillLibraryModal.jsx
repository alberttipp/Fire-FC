...
const DrillLibraryModal = ({ onClose, player, teamId }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
            <div className="bg-brand-dark border border-white/10 w-full max-w-4xl max-h-[90vh] rounded-xl shadow-2xl flex flex-col overflow-hidden">
                {/* Header */}
                <div className="bg-gray-900/50 p-5 border-b border-white/10 flex justify-between items-center shrink-0">
                    <div>
                        ...
                        <p className="text-xs text-gray-400 mt-1">
                            Browse and assign drills to {player?.first_name || 'player'} for solo practice
                        </p>
                    </div>
                ...
                <div className="p-4 border-b border-white/10 space-y-3 shrink-0">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full bg-black/30 border border-white/10 rounded-lg pl-10 pr-4 py-2.5 text-white text-sm focus:border-brand-green outline-none"
                        />
                    </div>
                    {skills.length > 0 && (
                        <div className="flex gap-2 flex-wrap items-center">
                            ...
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-xs font-bold uppercase transition-all ${filter === 'all' ? 'bg-brand-green text-brand-dark' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}
                            >
                                All ({drills.length})
                            </button>
                            {skills.map(s => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`px-3 py-1.5 rounded-full text-xs font-bold uppercase transition-all ${filter.toLowerCase() === s.toLowerCase() ? 'bg-brand-green text-brand-dark' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}
                                    >
                                        {s} ({count})
                                    </button>
                                );
